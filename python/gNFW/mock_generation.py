...
def mock_population_all(N, relT, relM, relRobs, relA,
    ...
    ages = np.random.uniform(8., 10., _N) # [yr] / [1-10 Gyr]
    # Mass
    mass = IMF_sampling(-0.6, _N, Mmin=0.0134, Mmax=0.0525) # [Msun] 14-55 Mjup
    ...
    #                (ages_wn > 8.) & (ages_wn < 9.998))
    #if len(pos[0]) < N:
    #    print(len(pos[0]))
    #    sys.exit("Less objects than required!")
    #print(np.min(ages[pos][:N]), np.max(ages[pos][:N]))
    #print(np.min(r_obs[pos][:N]), np.max(r_obs[pos][:N]))
    #return
    return (robs_wn, relRobs*r_obs,
            Tobs_wn, relT*Tobs,
            mass_wn, relM*mass,
            ages_wn, relA*ages)
