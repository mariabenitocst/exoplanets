...
solution = solver.MyModelPyMultiNest(Tobs, robs, sigmaTobs, sigmarobs, Mobs,
    ...
    b1_interp, c_interp, c1_interp, v, rho0, 
    n_dims=n_params, n_live_points=nlive, evidence_tolerance=tol,
    sampling_efficiency=0.7,
    outputfiles_basename="out/{}/".format(rank)+ex+ "_N{}_sigma{}_gamma{}_rs{}_v{}".format(nBDs, sigma, gamma_true, rs_true, rank),
    resume=False, verbose=False)
t1 = time.time()
print("Time taken to run 'PyMultiNest' is {} seconds".format(t1-t0))

#logZ_joint    = solution.logZ # logZ value
#logZerr_joint = solution.logZerr # estimated statistical uncertainty

# extract chains
#f_chain     = solution.samples[:, 0]
#gamma_chain = solution.samples[:, 1]
#rs_chain    = solution.samples[:, 2]
#samples     = np.vstack((f_chain, gamma_chain, rs_chain)).T

                                                                            
## save samples
#file_name   = "samplesMnest_" + ex + "_N{}_gamma{}_rs{}_v{}".format(nBDs, gamma_true, rs_true, rank)
